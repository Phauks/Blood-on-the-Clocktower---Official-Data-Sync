...
def extract_characters(page: Page) -> dict[str, dict]:
...
def extract_night_order(page: Page, characters: dict, selector: str, night_type: str) -> None:
    """Extract night order from first-night or other-night sheet.
    
    Args:
        page: Playwright page
        characters: Character dict to update
        selector: CSS selector for night sheet (.first-night or .other-night)
        night_type: "firstNight" or "otherNight"
    """
...
def extract_jinxes(page: Page, characters: dict) -> int:
    """Extract all jinxes from the Djinn section.
    
    Jinxes are stored bidirectionally on both characters.
...
def clean_character_data(characters: dict) -> dict:
    """Clean up character data before output.
    
    - Remove empty jinxes arrays
    
    Note: Internal fields (underscore-prefixed like _imageUrl) are NOT stripped here.
...
def filter_characters_by_edition(characters: dict, editions: list[str]) -> dict:
    """Filter characters to only include specified editions.
    
    Args:
        characters: Full character dict
