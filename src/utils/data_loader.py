...
def load_previous_character_data(characters_dir: Path | None = None) -> dict[str, dict]:
...
def load_character_file(char_file: Path) -> dict | None:
    ...
    Returns:
        Character dict or None if loading failed
    """
    try:
        with open(char_file, "r", encoding="utf-8") as f:
            return json.load(f)
...
def save_character_file(char_file: Path, character: dict) -> bool:
    """Save a character dict to JSON file.
...
def get_character_files_by_edition(edition: str, characters_dir: Path | None = None) -> list[Path]:
    ...
    Returns:
        Sorted list of Path objects for character files
    """
    if characters_dir is None:
        characters_dir = CHARACTERS_DIR
